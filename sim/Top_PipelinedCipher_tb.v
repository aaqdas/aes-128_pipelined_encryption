...
reg [NUM_PIPELINES*DATA_W-1:0] plain_text;
wire valid_out;
wire [NUM_PIPELINES*DATA_W-1:0] cipher_text;
reg dut_error;
reg [NUM_PIPELINES*DATA_W-1:0] data_expected;

reg [NUM_PIPELINES*DATA_W-1:0] data_input_vectors [0:No_Patterns-1];
reg [NUM_PIPELINES*DATA_W-1:0] cipherkey_input_vectors [0:No_Patterns-1];
reg [NUM_PIPELINES*DATA_W-1:0] output_vectors [0:No_Patterns-1];
...
initial begin
  ...
  for (i = 0; i < No_Patterns; i = i + 1) begin
    ...
    plain_text = {NUM_PIPELINES{data_input_vectors[i]}};
...
always @(posedge clk) begin
  if (valid_out && out_index < No_Patterns) begin
    data_expected = {NUM_PIPELINES{output_vectors[out_index]}};
